...
const TAG_ROWS = [
  [
    { id: "trade-tracking", icon: Search01Icon, label: "Trade Tracking" },
    { id: "entry-sizing", icon: UserGroupIcon, label: "Entry, Sizing, Conviction" },
    { id: "call-outcomes", icon: HierarchyIcon, label: "Call Outcome Quality" },
    { id: "position-context", icon: UserIcon, label: "Position Context" },
    { id: "post-exit-review", icon: RotateLeftIcon, label: "Post-Exit Review" },
  ],
  [
    { id: "market-data", icon: Settings02Icon, label: "Market Data" },
    { id: "snapshot-feed", icon: CpuIcon, label: "Snapshot + Feed Quality" },
    { id: "fx-context", icon: CodeIcon, label: "FX Context" },
    { id: "news-pricing", icon: Chart01Icon, label: "News + Pricing" },
    { id: "daily-surface", icon: FlashIcon, label: "Daily Market Surface" },
  ],
  [
    { id: "live-signals", icon: Link01Icon, label: "Live Signals" },
    { id: "signal-strength", icon: SmartPhone01Icon, label: "Signal Strength" },
    {
      id: "timing-routing",
      icon: CloudIcon,
      label: "Timing + Routing",
    },
    { id: "portfolios", icon: DatabaseIcon, label: "Portfolios" },
    { id: "exposure-drift", icon: LockIcon, label: "Exposure + P/L Drift" },
  ],
];

const CONFIG = {
  title: "Trentarev Capabilities",
  description:
...
const MagnifiedBento = ({ showFooter = false }: { showFooter?: boolean }) => {
  const containerRef = React.useRef<HTMLDivElement>(null);
  const lensX = useMotionValue(0);
  const lensY = useMotionValue(0);

  const clipPath = useMotionTemplate`circle(${CONFIG.revealRadius}px at calc(50% + ${lensX}px + ${CONFIG.lensFocusOffsetX}px) calc(50% + ${lensY}px + ${CONFIG.lensFocusOffsetY}px))`;
  const inverseMask = useMotionTemplate`radial-gradient(circle ${CONFIG.revealRadius}px at calc(50% + ${lensX}px + ${CONFIG.lensFocusOffsetX}px) calc(50% + ${lensY}px + ${CONFIG.lensFocusOffsetY}px), transparent 100%, black 100%)`;
      ...
      <div className="group relative w-full max-w-[420px] overflow-hidden rounded-[2rem] sm:rounded-[2.5rem] border bg-card p-1.5 sm:p-2 shadow-2xl shadow-primary/5 transition-all duration-500 hover:shadow-primary/10 hover:-translate-y-1">
        ...
        >
          <div className="relative h-full w-full flex flex-col items-center justify-center">
            ...
            <motion.div
              className="absolute top-1/2 left-1/2 z-40 cursor-grab active:cursor-grabbing drop-shadow-xl"
              drag
              dragMomentum={false}
            ...
            >
              <div className="relative">
                ...
                <div
                  ...
                  style={{
                    width: CONFIG.lensSize * 0.66,
                    height: CONFIG.lensSize * 0.66,
                    left: CONFIG.lensSize * 0.17,
                    top: CONFIG.lensSize * 0.17,
                    transform: `translate(${CONFIG.lensFocusOffsetX}px, ${CONFIG.lensFocusOffsetY}px)`,
                  }}
                />
              </div>
            </motion.div>
